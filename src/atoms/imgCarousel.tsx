...
export const ImgCarousel = (props: CarouselProps) => {
    const { img, indicators } = props;
    ...
    const setNext = (index: number, isNext: boolean) => {
        if (index > img.length - 1) index = img.length - 1;
        if (index < 0) index = 0;
...
const CarouselItem = ({ next, prev, swipe, state, index, maxIndex, child, height, setHeight }: CarouselItemProps) => {
    ...
    useEffect(() => {
        setHeight(divRef.current.offsetWidth * 2/3)
    }, [])
        ...
        <div style={{position: "absolute", width: '100%'}} ref={divRef}>
            <AnimatePresence custom={isNext}>
                <motion.div {...(swipe && dragProps)}>
                    ...
                    >
                        <Image width={height * 3/2} height={height} alt={child.description}
                               src={child.path !== "" ? child.path : "https://placehold.jp/300x200.png"}/>
                    </motion.div>
                </motion.div>
            </AnimatePresence>
        </div>
